...
def check_Choice(sheet, row, desc):
    try:
        key = sheet.cell(column=9, row=row).value
        key = char_num_re.sub(char_to_num, key)
    except (Exception) as e:
        return False, str(e)

    return option_checker(sheet, row, desc)


def option_checker(sheet, row, desc):
    idx = 10
...
def update_DB_from_excel(excel_url, db_name):
    ...
    wb = openpyxl.load_workbook(
        filename=excel_url, read_only=True, data_only=True)
    # sectionID
    # description
            ...
            while ques_desc != "None":
                try:
                    ...
                    if ques_type == "None" or "#N/A":
                        ques_type = type_dict[str(
                            cur_sht['E' + str(row_idx)].value)]
                    
                    lesson_desc = cur_sht['A' + str(row_idx)].value
                    lesson = lesson_list.get(description=lesson_desc)
                    
                    parsefunc = getattr(
                        sys.modules[__name__], "parse_" + ques_type)
                    ...
                    print("---- " + e, ques_desc,
                          lesson_desc, row_idx, ques_type)
                except (excepts.ObjectDoesNotExist) as e:
                    print("??? ", str(e), ques_desc,
                          lesson_desc, row_idx, ques_type)
...
def import_lesson_list(excel_url, db_name):
    ...
    if lesson_list_sht:
        ...
        while lesson_desc:
            try:
                questionModels.Lesson.objects.using(
                    db_name).create(description=lesson_desc)
            except (Exception) as e:
                print("---- " + str(e))
            row_idx += 1
            lesson_desc = lesson_list_sht['A' + str(row_idx)].value
...
def check_question_excel(excel_url, db_name):
    ...
    if config_sht:
        ...
        for row in range(2, 8):
            qTypeName = str(config_sht.cell(row=row, column=4).value)
            type_dict[qTypeName] = str(
                config_sht.cell(row=row, column=5).value)
        ...
        for row in range(2, 8):
            ...
            if check_sheet:
                ...
                while ques_desc != "None":
                    ...
                    if not ques_type == qTypeName:
                        check_string = check_string + "\t第" + \
                            str(row_idx) + "行 第E列，题目类型和页面类型不一致：" + ques_type 

                    if ques_type == "None" or "#N/A":
                        ques_type = type_dict[str(
                            check_sheet['E' + str(row_idx)].value)]
                    ...
                    if lesson_list.filter(description=lesson_desc).exists():
                        try:
                            checkfunc = getattr(
                                sys.modules[__name__], "check_" + ques_type)
                            ret_v, ret_str = checkfunc(
                                check_sheet, row_idx, ques_desc)
                            valid = valid and ret_v
                            check_string += ret_str
                        except (AttributeError) as e:
                            check_string = check_string + "\t第" + \
                                str(row_idx) + "行 |题目类型|：<" + \
                                str(ques_type) + "> 错误! 错误信息：" + str(e) + "\n"
                            valid = False
                        except (Exception) as e:
                            print(str(e))
                            check_string = check_string + "\t第" + \
                                str(row_idx) + "行 <" + str(ques_type) + \
                                ">题，题目有错  错误信息：" + str(e) + "\n"
                            valid = False
                    else:
                        check_string = check_string + "\t第" + \
                            str(row_idx) + "行 第一列，所属单元:" + \
                            str(lesson_desc) + " 错误或者不存在!\n"
                        valid = False
